  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/auth/register" && request.method === "POST") {
      try {
        ...
        const { hashHex, saltBase64Out } = await hashPassword(password);
        const id = uuid();
        await env.DB.prepare(
          "INSERT INTO users (id, username, password_hash, salt, display_name) VALUES (?, ?, ?, ?, ?)"
        )
          .bind(id, username, hashHex, saltBase64Out, displayName)
          .run();
    ...
    if (url.pathname === "/posts" && request.method === "GET") {
      ...
      if (cursor) {
        const cursorRow = await env.DB.prepare("SELECT created_at FROM posts WHERE id = ?").bind(cursor).first();
        const cursorCreated = cursorRow && typeof cursorRow === "object" ? (cursorRow as Record<string, unknown>).created_at : null;
        if (cursorCreated) {
          const s = await env.DB.prepare("SELECT p.id, p.user_id, p.content, p.image_urls, p.created_at, p.updated_at, u.username, u.display_name, u.avatar_url FROM posts p JOIN users u ON p.user_id = u.id WHERE p.created_at < ? ORDER BY p.created_at DESC LIMIT ?").bind(cursorCreated, limit).all();
          results = s.results as Record<string, unknown>[];
        } else {
          const s = await env.DB.prepare("SELECT p.id, p.user_id, p.content, p.image_urls, p.created_at, p.updated_at, u.username, u.display_name, u.avatar_url FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC LIMIT ?").bind(limit).all();
          results = s.results as Record<string, unknown>[];
        }
      } else {
        const s = await env.DB.prepare("SELECT p.id, p.user_id, p.content, p.image_urls, p.created_at, p.updated_at, u.username, u.display_name, u.avatar_url FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC LIMIT ?").bind(limit).all();
      ...
      const posts = await Promise.all(
        results.map(async (r) => {
          ...
          const likeCount = (await env.DB.prepare("SELECT COUNT(*) as c FROM post_likes WHERE post_id = ?").bind(postId).first()) as { c: number };
          let liked = false;
          if (userId) {
            const l = await env.DB.prepare("SELECT 1 FROM post_likes WHERE post_id = ? AND user_id = ?").bind(postId, userId).first();
            liked = !!l;
          }
          const commentCount = (await env.DB.prepare("SELECT COUNT(*) as c FROM comments WHERE post_id = ?").bind(postId).first()) as { c: number };
          return {
            id: r.id,
            user_id: r.user_id,
            content: r.content,
    ...
    if (likePostMatch) {
      ...
      if (request.method === "POST") {
        ...
        const count = (await env.DB.prepare("SELECT COUNT(*) as c FROM post_likes WHERE post_id = ?").bind(postId).first()) as { c: number };
    ...
    if (postLikesMatch && request.method === "GET") {
      ...
      const userId = await getUserIdFromRequest(request, secret);
      const count = (await env.DB.prepare("SELECT COUNT(*) as c FROM post_likes WHERE post_id = ?").bind(postId).first()) as { c: number };
      const liked = userId ? (await env.DB.prepare("SELECT 1 FROM post_likes WHERE post_id = ? AND user_id = ?").bind(postId, userId).first()) : null;
    ...
    if (commentsPostMatch) {
      ...
      if (request.method === "POST") {
        ...
        if (!content) return jsonResponse({ error: "内容不能为空" }, 400);
        const id = uuid();
        await env.DB.prepare("INSERT INTO comments (id, post_id, user_id, content) VALUES (?, ?, ?, ?)").bind(id, postId, uid, content).run();
        const row = await env.DB.prepare(
    ...
    if (url.pathname === "/follows" && request.method === "POST") {
      ...
      await env.DB.prepare("INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)").bind(userId, followingId).run();
    ...
    if (unfollowMatch && request.method === "DELETE") {
      ...
      const userId = await getUserIdFromRequest(request, secret);
      if (!userId) return jsonResponse({ error: "未登录" }, 401);
      await env.DB.prepare("DELETE FROM follows WHERE follower_id = ? AND following_id = ?").bind(userId, followingId).run();
    ...
    if (url.pathname === "/messages" && request.method === "GET") {
      ...
      const { results: convList } = await env.DB.prepare(
    ...
    if (url.pathname === "/messages" && request.method === "POST") {
      ...
      const id = uuid();
      await env.DB.prepare("INSERT INTO messages (id, sender_id, receiver_id, content, read) VALUES (?, ?, ?, ?, 0)").bind(id, userId, receiverId, content).run();
      const row = await env.DB.prepare("SELECT id, sender_id, receiver_id, content, created_at, read FROM messages WHERE id = ?").bind(id).first();
