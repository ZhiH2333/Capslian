  ...
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (pathname === "/users/me" && request.method === "PATCH") {
      ...
      try {
        ...
        values.push(userId);
        await env.molian_db.prepare(`UPDATE users SET ${updates.join(", ")} WHERE id = ?`).bind(...values).run();
        const row = await env.molian_db.prepare(
          "SELECT id, username, display_name, avatar_url, bio, created_at FROM users WHERE id = ?"
        )
          .bind(userId)
    ...
    if (pathname === "/posts" && request.method === "GET") {
      ...
      if (cursor) {
        ...
        if (cursorCreated) {
          const s = await env.molian_db.prepare("SELECT p.id, p.user_id, p.content, p.image_urls, p.created_at, p.updated_at, u.username, u.display_name, u.avatar_url FROM posts p JOIN users u ON p.user_id = u.id WHERE p.created_at < ? ORDER BY p.created_at DESC LIMIT ?").bind(cursorCreated, limit).all();
          results = s.results as Record<string, unknown>[];
        } else {
          const s = await env.molian_db.prepare("SELECT p.id, p.user_id, p.content, p.image_urls, p.created_at, p.updated_at, u.username, u.display_name, u.avatar_url FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC LIMIT ?").bind(limit).all();
          results = s.results as Record<string, unknown>[];
        }
      } else {
        const s = await env.molian_db.prepare("SELECT p.id, p.user_id, p.content, p.image_urls, p.created_at, p.updated_at, u.username, u.display_name, u.avatar_url FROM posts p JOIN users u ON p.user_id = u.id ORDER BY p.created_at DESC LIMIT ?").bind(limit).all();
    ...
    if (postIdMatch) {
      ...
      if (request.method === "DELETE") {
        const userId = await getUserIdFromRequest(request, secret);
        if (!userId) return jsonResponse({ error: "未登录" }, 401);
        const row = await env.molian_db.prepare("SELECT user_id FROM posts WHERE id = ?").bind(id).first() as { user_id: string } | null;
        if (!row) return jsonResponse({ error: "帖子不存在" }, 404);
        if (row.user_id !== userId) return jsonResponse({ error: "只能删除自己的帖子" }, 403);
        await env.molian_db.prepare("DELETE FROM post_likes WHERE post_id = ?").bind(id).run();
        await env.molian_db.prepare("DELETE FROM comments WHERE post_id = ?").bind(id).run();
        await env.molian_db.prepare("DELETE FROM posts WHERE id = ?").bind(id).run();
    ...
    if (likePostMatch) {
      ...
      if (request.method === "POST") {
        await env.molian_db.prepare("INSERT OR IGNORE INTO post_likes (post_id, user_id) VALUES (?, ?)").bind(postId, userId).run();
        const count = (await env.molian_db.prepare("SELECT COUNT(*) as c FROM post_likes WHERE post_id = ?").bind(postId).first()) as { c: number };
        return jsonResponse({ liked: true, count: count?.c ?? 0 });
      }
      if (request.method === "DELETE") {
        await env.molian_db.prepare("DELETE FROM post_likes WHERE post_id = ? AND user_id = ?").bind(postId, userId).run();
        const count = (await env.molian_db.prepare("SELECT COUNT(*) as c FROM post_likes WHERE post_id = ?").bind(postId).first()) as { c: number };
    ...
    if (postLikesMatch && request.method === "GET") {
      ...
      const userId = await getUserIdFromRequest(request, secret);
      const count = (await env.molian_db.prepare("SELECT COUNT(*) as c FROM post_likes WHERE post_id = ?").bind(postId).first()) as { c: number };
      const liked = userId ? (await env.molian_db.prepare("SELECT 1 FROM post_likes WHERE post_id = ? AND user_id = ?").bind(postId, userId).first()) : null;
    ...
    if (commentsPostMatch) {
      ...
      if (request.method === "GET") {
        const limit = Math.min(Number(url.searchParams.get("limit")) || 20, 100);
        const { results } = await env.molian_db.prepare(
      ...
      if (request.method === "POST") {
        ...
        if (!content) return jsonResponse({ error: "内容不能为空" }, 400);
        const id = uuid();
        await env.molian_db.prepare("INSERT INTO comments (id, post_id, user_id, content) VALUES (?, ?, ?, ?)").bind(id, postId, uid, content).run();
        const row = await env.molian_db.prepare(
    ...
    if (pathname === "/follows" && request.method === "POST") {
      ...
      await env.molian_db.prepare("INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)").bind(userId, followingId).run();
    ...
    if (unfollowMatch && request.method === "DELETE") {
      ...
      const userId = await getUserIdFromRequest(request, secret);
      if (!userId) return jsonResponse({ error: "未登录" }, 401);
      await env.molian_db.prepare("DELETE FROM follows WHERE follower_id = ? AND following_id = ?").bind(userId, followingId).run();
    ...
    if (pathname === "/friend-requests" && request.method === "POST") {
      ...
      try {
        const existing = await env.molian_db.prepare(
        ...
        if (msg.includes("no such table") || msg.includes("friend_requests")) {
          return jsonResponse({ error: "服务未就绪，请先执行数据库迁移：wrangler d1 migrations apply molian-db --remote" }, 503);
        }
    ...
    if (friendRequestAcceptMatch && request.method === "POST") {
      ...
      try {
        ...
        await env.molian_db.prepare("UPDATE friend_requests SET status = 'accepted' WHERE id = ?").bind(requestId).run();
        await env.molian_db.prepare("INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)").bind(r.requester_id, r.target_id).run();
        await env.molian_db.prepare("INSERT OR IGNORE INTO follows (follower_id, following_id) VALUES (?, ?)").bind(r.target_id, r.requester_id).run();
    ...
    if (friendRequestRejectMatch && request.method === "POST") {
      ...
      try {
        const row = await env.molian_db.prepare(
          ...
          .first();
        if (!row || typeof row !== "object") return jsonResponse({ error: "申请不存在或已处理" }, 404);
        await env.molian_db.prepare("UPDATE friend_requests SET status = 'rejected' WHERE id = ?").bind(requestId).run();
    ...
    const deleteFriendMatch = pathname.match(/^\/users\/me\/friends\/([^/]+)$/);
    if (deleteFriendMatch && request.method === "DELETE") {
      const userId = await getUserIdFromRequest(request, secret);
      if (!userId) return jsonResponse({ error: "未登录" }, 401);
      const friendId = deleteFriendMatch[1];
      try {
        const result = await env.molian_db.prepare(
        ...
        if (rowsWritten === 0) {
          return jsonResponse({ error: "不是好友或已删除" }, 404);
        }
        return jsonResponse({});
      } catch (e) {
    ...
    if (pathname === "/messages" && request.method === "GET") {
      ...
      if (withUser) {
        ...
        const { results } = await env.molian_db.prepare(sql).bind(...bind).all();
        return jsonResponse({ messages: results });
      }
      const { results: convList } = await env.molian_db.prepare(
        ...
        (convList as { peer_id: string }[]).map(async (row) => {
          ...
          const [last, unread, peerRow] = await Promise.all([
            ...
            env.molian_db.prepare(
              ...
              .bind(userId, peerId)
              .first(),
            env.molian_db.prepare("SELECT username, display_name FROM users WHERE id = ?")
              .bind(peerId)
              .first(),
          ]);
    ...
    if (pathname === "/messages" && request.method === "POST") {
      ...
      const id = uuid();
      await env.molian_db.prepare("INSERT INTO messages (id, sender_id, receiver_id, content, read) VALUES (?, ?, ?, ?, 0)").bind(id, userId, receiverId, content).run();
      const row = await env.molian_db.prepare("SELECT id, sender_id, receiver_id, content, created_at, read FROM messages WHERE id = ?").bind(id).first();
